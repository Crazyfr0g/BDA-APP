...
class ShowDrought extends StatefulWidget {
  ShowDrought(
      this.typeOfUser,
  ...
  @override
  _ShowDroughtState createState() => _ShowDroughtState();
}

class _ShowDroughtState extends State<ShowDrought> {
  String typeOfUser;
  ...
  void initState() {
    super.initState();
    _knowNameOfUser();
    _buildDroughtRequest();
  }


  Widget _buildDroughtRequest() {
    if (widget.typeOfUser == "Barangay") {
      return DroughtFeedBeneficiary(widget.mycontroller);
    }
    else if (widget.typeOfUser == "Normal User") {
      return DroughtFeedBenefactor(widget.availableDonations, widget.totalInterestedCount, widget.totalConfirmedHelpCount, widget.mycontroller, widget.nameOfUser);
    }
    else if (widget.typeOfUser == "Admin"){
      return DroughtFeedAdmin(widget.availableDonations, widget.totalInterestedCount, widget.totalConfirmedHelpCount, widget.mycontroller);
    ...
    Widget StreamBuider(){
      ...
      return
        StreamBuilder<QuerySnapshot>(
            stream: db.collection('DROUGHT REQUESTS').orderBy('Help_DatePosted', descending: true).snapshots(),
      ...
      return
        StreamBuilder<QuerySnapshot>(
            stream: db.collection('DROUGHT REQUESTS').where('Name_ofUser', isEqualTo: 'Barangay ${widget.mycontroller.text}').snapshots(),
          builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return Column(
                      children: snapshot.data.documents
...
class DroughtFeedBenefactorAnswerHelp extends StatefulWidget {
  DroughtFeedBenefactorAnswerHelp(
  ...
  @override
  _DroughtFeedBenefactorAnswerHelpState createState() => _DroughtFeedBenefactorAnswerHelpState();

}

class _DroughtFeedBenefactorAnswerHelpState extends State<DroughtFeedBenefactorAnswerHelp> {
  ...
  _saveConfirmedCountToUser() async {
    ...
    DocumentReference ref = db.collection('USERS').document(widget._requestorsID);
  ...
  _saveConfirmedCountToBeneficiaryRespondentsDroughtRequest() async {
    ...
    DocumentReference ref = db.collection('DROUGHT REQUESTS').document(widget._requestorsuniqueID);
  ...
  _saveConfirmedHelpCount() async {
    ...
    ref.get().then((document) async{
      if (document.exists) {
        setState(() async {
          ref.updateData({
            'Total_ConfirmedHelp': widget._totalRespondentsHelpCount + count,
          });
        });
      }
      else
...
class _DroughtFeedBeneficiaryState extends State<DroughtFeedBeneficiary> {
  ...
  void countDocuments() async {
    QuerySnapshot _myDoc = await Firestore.instance.collection('HELP REQUEST').where('User_ID', ).getDocuments();
                      ...
                      children: <Widget> [
                        ...
                        Column(
                          ...
                          children: <Widget>[
                            ...
                            Row(
                              children: <Widget>[
//                                Text(doc.data['Help_DatePosted'].toDate().toString(),
                                Text(Dformat.format(doc.data['Help_DatePosted'].toDate()),
                                  style: TextStyle(
                                      fontSize: 9,
                                      fontWeight: FontWeight.w800
  ...
  Widget build(BuildContext context) {
    ...
    Widget StreamBuider(){
      ...
      final name = widget.mycontroller.toString();
