...
class ShowFlood extends StatefulWidget {
  ShowFlood(
      this.typeOfUser,
  ...
  @override
  _ShowFloodState createState() => _ShowFloodState();
}

class _ShowFloodState extends State<ShowFlood> {
  String typeOfUser;
  ...
  Widget _buildFloodRequest() {
    if (widget.typeOfUser == "Barangay") {
      return FloodFeedBeneficiary(widget.mycontroller);
    }
    else if (widget.typeOfUser == "Normal User") {
      return FloodFeedBenefactor(widget.availableDonations, widget.totalInterestedCount, widget.totalConfirmedHelpCount, widget.mycontroller, widget.nameOfUser);
    }
    else if (widget.typeOfUser == "Admin"){
      return FloodFeedAdmin(widget.availableDonations, widget.totalInterestedCount, widget.totalConfirmedHelpCount, widget.mycontroller);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _buildFloodRequest(),
    );
  }
}


class FloodFeedBenefactor extends StatefulWidget {
  FloodFeedBenefactor(
      this.availableDonations,
      this.totalInterestedCount,
      this.totalConfirmedhelpCount,
  ...
  @override
  _FloodFeedBenefactorState createState() => _FloodFeedBenefactorState();
}

class _FloodFeedBenefactorState extends State<FloodFeedBenefactor> {
  @override
                                ...
                                onPressed: () async {
                                  ...
                                  if (user.isEmailVerified){
                                    Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                            builder: (BuildContext context) => FloodFeedBenefactorAnswerHelp(
                                                doc.data['Help_Description'],
                                                doc.data['Name_ofUser'],
                                                doc.data['User_ID'],
                                                doc.data['Help_DropoffLocation'],
  ...
  Widget build(BuildContext context) {
    ...
    Widget StreamBuider(){
      if (widget.mycontroller.text == ""){
        return
          StreamBuilder<QuerySnapshot>(
              stream: db.collection('FLOOD REQUESTS').orderBy('Help_DatePosted', descending: true).snapshots(),
      ...
      else
      {
        return
          StreamBuilder<QuerySnapshot>(
              stream: db.collection('FLOOD REQUESTS').where('Name_ofUser', isEqualTo: 'Barangay ${widget.mycontroller.text}').snapshots(),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return Column(
                      children: snapshot.data.documents
...
class FloodFeedBenefactorAnswerHelp extends StatefulWidget {
  FloodFeedBenefactorAnswerHelp(
  ...
  @override
  _FloodFeedBenefactorAnswerHelpState createState() => _FloodFeedBenefactorAnswerHelpState();
}

class _FloodFeedBenefactorAnswerHelpState extends State<FloodFeedBenefactorAnswerHelp> {
  ...
  _saveConfirmedCountToBeneficiaryRespondentsDroughtRequest() async {
    ...
    DocumentReference ref = db.collection('FLOOD REQUESTS').document(widget._requestorsuniqueID);
...
class FloodFeedBeneficiary extends StatefulWidget {
  FloodFeedBeneficiary(
      this.mycontroller) : super();
  final mycontroller;

  @override
  _FloodFeedBeneficiaryState createState() => _FloodFeedBeneficiaryState();
}

class _FloodFeedBeneficiaryState extends State<FloodFeedBeneficiary> {
  @override
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      ...
                      children: <Widget>[
                        CircleAvatar(
                            radius: 20,
                            backgroundImage: AssetImage('assets/Flood.jpg')
                        ),
                      ],
                    ),
                  ],
...
class FloodFeedAdmin extends StatefulWidget {
  FloodFeedAdmin(
      this.availableDonations,
      this.totalInterestedCount,
      this.totalConfirmedhelpCount,
  ...
  @override
  _FloodFeedAdminState createState() => _FloodFeedAdminState();
}

class _FloodFeedAdminState extends State<FloodFeedAdmin> {
  @override
