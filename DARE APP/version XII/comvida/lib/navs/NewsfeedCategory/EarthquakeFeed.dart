...
class ShowEarthquake extends StatefulWidget {
  ShowEarthquake(
      this.typeOfUser,
  ...
  @override
  _ShowEarthquakeState createState() => _ShowEarthquakeState();
}

class _ShowEarthquakeState extends State<ShowEarthquake> {
  String typeOfUser;
  ...
  void initState() {
    super.initState();
    _knowNameOfUser();
    _buildEarthquakeRequest();
  }



  Widget _buildEarthquakeRequest() {
    if (widget.typeOfUser == "Barangay") {
      return EarthquakeFeedBeneficiary(widget.mycontroller);
    }
    else if (widget.typeOfUser == "Normal User") {
      return EarthquakeFeedBenefactor(widget.availableDonations, widget.totalInterestedCount, widget.totalConfirmedHelpCount, widget.mycontroller, widget.nameOfUser);
    }
    else if (widget.typeOfUser == "Admin"){
      return EarthquakeFeedAdmin(widget.availableDonations, widget.totalInterestedCount, widget.totalConfirmedHelpCount, widget.mycontroller);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: _buildEarthquakeRequest(),
    );
  }
}


class EarthquakeFeedBenefactor extends StatefulWidget {
  EarthquakeFeedBenefactor(
      this.availableDonations,
      this.totalInterestedCount,
      this.totalConfirmedhelpCount,
      this.mycontroller,
  ...
  @override
  _EarthquakeFeedBenefactorState createState() => _EarthquakeFeedBenefactorState();
}

class _EarthquakeFeedBenefactorState extends State<EarthquakeFeedBenefactor> {
                                ...
                                onPressed: () async {
                                  ...
                                  if (user.isEmailVerified){
                                    Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                            builder: (BuildContext context) => EarthquakeFeedBenefactorAnswerHelp(
                                                doc.data['Help_Description'],
                                                doc.data['Name_ofUser'],
                                                doc.data['User_ID'],
                                                doc.data['Help_DropoffLocation'],
  ...
  Widget build(BuildContext context) {
    ...
    Widget StreamBuider(){
      if (widget.mycontroller.text == ""){
        return
          StreamBuilder<QuerySnapshot>(
              stream: db.collection('EARTHQUAKE REQUESTS').orderBy('Help_DatePosted', descending: true).snapshots(),
      ...
      else
      {
        return
          StreamBuilder<QuerySnapshot>(
              stream: db.collection('EARTHQUAKE REQUESTS').where('Name_ofUser', isEqualTo: 'Barangay ${widget.mycontroller.text}').snapshots(),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return Column(
                      children: snapshot.data.documents
...
class EarthquakeFeedBenefactorAnswerHelp extends StatefulWidget {
  EarthquakeFeedBenefactorAnswerHelp(
  ...
  @override
  _EarthquakeFeedBenefactorAnswerHelpState createState() => _EarthquakeFeedBenefactorAnswerHelpState();
}

class _EarthquakeFeedBenefactorAnswerHelpState extends State<EarthquakeFeedBenefactorAnswerHelp> {
  ...
  _saveConfirmedCountToBeneficiaryRespondentsDroughtRequest() async {
    ...
    DocumentReference ref = db.collection('EARTHQUAKE REQUESTS').document(widget._requestorsuniqueID);
...
class EarthquakeFeedBeneficiary extends StatefulWidget {
  EarthquakeFeedBeneficiary(
      this.mycontroller) : super();
  final mycontroller;

  @override
  _EarthquakeFeedBeneficiaryState createState() => _EarthquakeFeedBeneficiaryState();
}

class _EarthquakeFeedBeneficiaryState extends State<EarthquakeFeedBeneficiary> {
  @override
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      ...
                      children: <Widget>[
                        CircleAvatar(
                            radius: 20,
                            backgroundImage: AssetImage('assets/Earthquake.jpg')
                        ),
                      ],
                    ),
                  ],
...
class EarthquakeFeedAdmin extends StatefulWidget {
  EarthquakeFeedAdmin(
      this.availableDonations,
      this.totalInterestedCount,
      this.totalConfirmedhelpCount,
  ...
  @override
  _EarthquakeFeedAdminState createState() => _EarthquakeFeedAdminState();
}

class _EarthquakeFeedAdminState extends State<EarthquakeFeedAdmin> {
  @override
