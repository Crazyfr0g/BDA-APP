...
class ShowLandslide extends StatefulWidget {
  ShowLandslide(
      this.typeOfUser,
  ...
  @override
  _ShowLandslideState createState() => _ShowLandslideState();
}

class _ShowLandslideState extends State<ShowLandslide> {
  String typeOfUser;
  ...
  void initState() {
    super.initState();
    _knowNameOfUser();
    _buildLandslideRequest();
  }

  Widget _buildLandslideRequest() {
    if (widget.typeOfUser == "Barangay") {
      return LandslideFeedBeneficiary(widget.mycontroller);
    }
    else if (widget.typeOfUser == "Normal User") {
      return LandslideFeedBenefactor(widget.availableDonations, widget.totalInterestedCount, widget.totalConfirmedHelpCount, widget.mycontroller, widget.nameOfUser);
    }
    else if (widget.typeOfUser == "Admin"){
      return LandslideFeedAdmin(widget.availableDonations, widget.totalInterestedCount, widget.totalConfirmedHelpCount, widget.mycontroller);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: _buildLandslideRequest(),
    );
  }
}


class LandslideFeedBenefactor extends StatefulWidget {
  LandslideFeedBenefactor(
      this.availableDonations,
      this.totalInterestedCount,
      this.totalConfirmedhelpCount,
      this.mycontroller,
  ...
  @override
  _LandslideFeedBenefactorState createState() => _LandslideFeedBenefactorState();
}

class _LandslideFeedBenefactorState extends State<LandslideFeedBenefactor> {
  @override
                                ...
                                onPressed: () async {
                                  ...
                                  if (user.isEmailVerified){
                                    Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                            builder: (BuildContext context) => LandslideFeedBenefactorAnswerHelp(
                                                doc.data['Help_Description'],
                                                doc.data['Name_ofUser'],
                                                doc.data['User_ID'],
                                                doc.data['Help_DropoffLocation'],
  ...
  Widget build(BuildContext context) {
    ...
    Widget StreamBuider(){
      ...
      if (widget.mycontroller.text == ""){
        return
          StreamBuilder<QuerySnapshot>(
              stream: db.collection('LANDSLIDE REQUESTS').orderBy('Help_DatePosted', descending: true).snapshots(),
      ...
      else
      {
        return
          StreamBuilder<QuerySnapshot>(
              stream:  db.collection('LANDSLIDE REQUESTS').where('Name_ofUser', isEqualTo: 'Barangay ${widget.mycontroller.text}').snapshots(),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return Column(
                      children: snapshot.data.documents
...
class LandslideFeedBenefactorAnswerHelp extends StatefulWidget {
  LandslideFeedBenefactorAnswerHelp(
  ...
  @override
  _LandslideFeedBenefactorAnswerHelpState createState() => _LandslideFeedBenefactorAnswerHelpState();
}

class _LandslideFeedBenefactorAnswerHelpState extends State<LandslideFeedBenefactorAnswerHelp> {
  ...
  _saveConfirmedCountToBeneficiaryRespondentsDroughtRequest() async {
    ...
    DocumentReference ref = db.collection('LANDSLIDE REQUESTS').document(widget._requestorsuniqueID);
...
class LandslideFeedBeneficiary extends StatefulWidget {
  LandslideFeedBeneficiary(
      this.mycontroller) : super();
  final mycontroller;

  @override
  _LandslideFeedBeneficiaryState createState() => _LandslideFeedBeneficiaryState();
}

class _LandslideFeedBeneficiaryState extends State<LandslideFeedBeneficiary> {
  @override
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      ...
                      children: <Widget>[
                        CircleAvatar(
                            radius: 20,
                            backgroundImage: AssetImage('assets/Landslide.jpg')
                        ),
                      ],
                    ),
                  ],
...
class LandslideFeedAdmin extends StatefulWidget {
  LandslideFeedAdmin(
      this.availableDonations,
      this.totalInterestedCount,
      this.totalConfirmedhelpCount,
  ...
  @override
  _LandslideFeedAdminState createState() => _LandslideFeedAdminState();
}

class _LandslideFeedAdminState extends State<LandslideFeedAdmin> {
  @override
