...
class ShowTyphoon extends StatefulWidget {
  ShowTyphoon(
      this.typeOfUser,
  ...
  @override
  _ShowTyphoonState createState() => _ShowTyphoonState();
}

class _ShowTyphoonState extends State<ShowTyphoon> {
  ...
  void initState() {
    super.initState();
    _knowNameOfUser();
    _buildTyphoonRequest();
  }


  Widget _buildTyphoonRequest() {
    if (widget.typeOfUser == "Barangay") {
      return TyphoonFeedBeneficiary(widget.mycontroller);
    }
    else if (widget.typeOfUser == "Normal User") {
      return TyphoonFeedBenefactor(widget.availableDonations, widget.totalInterestedCount, widget.totalConfirmedHelpCount, widget.mycontroller, widget.nameOfUser);
    }
    else if (widget.typeOfUser == "Admin"){
      return TyphoonFeedAdmin(widget.availableDonations, widget.totalInterestedCount, widget.totalConfirmedHelpCount, widget.mycontroller);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: _buildTyphoonRequest(),
    );
  }
}


class TyphoonFeedBenefactor extends StatefulWidget {
  TyphoonFeedBenefactor(
      this.availableDonations,
      this.totalInterestedCount,
      this.totalConfirmedhelpCount,
      this.mycontroller,
  ...
  @override
  _TyphoonFeedBenefactorState createState() => _TyphoonFeedBenefactorState();
}

class _TyphoonFeedBenefactorState extends State<TyphoonFeedBenefactor> {
  @override
                                ...
                                onPressed: () async {
                                  ...
                                  if (user.isEmailVerified){
                                    Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                            builder: (BuildContext context) => TyphoonFeedBenefactorAnswerHelp(
                                                doc.data['Help_Description'],
                                                doc.data['Name_ofUser'],
                                                doc.data['User_ID'],
                                                doc.data['Help_DropoffLocation'],
  ...
  Widget build(BuildContext context) {
    ...
    Widget StreamBuider(){
      if (widget.mycontroller.text == ""){
        return
          StreamBuilder<QuerySnapshot>(
              stream: db.collection('TYPHOON REQUESTS').orderBy('Help_DatePosted', descending: true).snapshots(),
      ...
      else
      {
        return
          StreamBuilder<QuerySnapshot>(
              stream: db.collection('TYPHOON REQUESTS').where('Name_ofUser', isEqualTo: 'Barangay ${widget.mycontroller.text}').snapshots(),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return Column(
                      children: snapshot.data.documents
...
class TyphoonFeedBenefactorAnswerHelp extends StatefulWidget {
  TyphoonFeedBenefactorAnswerHelp(
  ...
  @override
  _TyphoonFeedBenefactorAnswerHelpState createState() => _TyphoonFeedBenefactorAnswerHelpState();
}

class _TyphoonFeedBenefactorAnswerHelpState extends State<TyphoonFeedBenefactorAnswerHelp> {
  void initState() {
    super.initState();
...
class TyphoonFeedBeneficiary extends StatefulWidget {
  TyphoonFeedBeneficiary(
      this.mycontroller) : super();
  final mycontroller;

  @override
  _TyphoonFeedBeneficiaryState createState() => _TyphoonFeedBeneficiaryState();
}

class _TyphoonFeedBeneficiaryState extends State<TyphoonFeedBeneficiary>  {
  @override
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      ...
                      children: <Widget>[
                        CircleAvatar(
                            radius: 20,
                            backgroundImage: AssetImage('assets/Typhoon.jpg')
                        ),
                      ],
                    ),
                  ],
...
class TyphoonFeedAdmin extends StatefulWidget {
  TyphoonFeedAdmin(
      this.availableDonations,
      this.totalInterestedCount,
      this.totalConfirmedhelpCount,
  ...
  @override
  _TyphoonFeedAdminState createState() => _TyphoonFeedAdminState();
}

class _TyphoonFeedAdminState extends State<TyphoonFeedAdmin> {
  @override
